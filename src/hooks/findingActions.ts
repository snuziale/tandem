...
export function pendingCommentOf(finding: Finding, bodyOverride?: string): Omit<PendingComment, 'localId'> {
  return {
...
export async function acceptFinding(
  ...
  try {
    if (opts.editedBody !== undefined && opts.editedBody !== commentBodyOf(finding)) {
    ...
    toast.error('Could not stage finding', { description: e instanceof Error ? e.message : undefined });
  } finally {
    queryClient.invalidateQueries({ queryKey: ['runs'] });
  }
}

export async function dismissFinding(queryClient: QueryClient, finding: Finding): Promise<void> {
  try {
    await setFindingState(finding.runId, finding.id, 'dismissed');
  } catch (e) {
    toast.error('Could not dismiss finding', { description: e instanceof Error ? e.message : undefined });
  } finally {
    queryClient.invalidateQueries({ queryKey: ['runs'] });
  }
}
...
export async function unstageFinding(queryClient: QueryClient, runId: string, findingId: string): Promise<void> {
  try {
