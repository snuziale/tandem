...
export async function submitReview(creds: GitHubCreds, ref: PrRef, input: SubmitReviewInput): Promise<{ reviewId: number; url: string }> {
  const { data } = await rest<{ id: number; html_url: string }>(creds, `/repos/${ref.owner}/${ref.repo}/pulls/${ref.number}/reviews`, {
    method: 'POST',
    body: {
...
export function quickApprove(creds: GitHubCreds, ref: PrRef): Promise<{ reviewId: number; url: string }> {
  return submitReview(creds, ref, { verdict: 'APPROVE', body: '', comments: [] });
}
