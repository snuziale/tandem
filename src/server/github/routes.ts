...
async function handleSubmit(req: Request, ref: PrRef): Promise<Response> {
  ...
  if (!isPlainObject(body) || (body.verdict !== 'APPROVE' && body.verdict !== 'REQUEST_CHANGES' && body.verdict !== 'COMMENT')) {
    return Response.json({ error: 'expected { verdict: APPROVE|REQUEST_CHANGES|COMMENT, summaryBody? }' }, { status: 400 });
  }
  const verdict = body.verdict;
  const summaryBody = typeof body.summaryBody === 'string' ? body.summaryBody : '';
  ...
  if (comments.length === 0 && verdict === 'COMMENT' && !summaryBody.trim()) {
    return Response.json({ error: 'nothing to submit — stage a comment or write a summary' }, { status: 400 });
  }
...
function restCommentOf(c: PendingComment) {
  const body = c.suggestion !== undefined ? `${c.body}${c.body.trim() ? '\n\n' : ''}\`\`\`suggestion\n${c.suggestion}\n\`\`\`` : c.body;
  return {
    path: c.path,
    line: c.line,
    side: c.side,
