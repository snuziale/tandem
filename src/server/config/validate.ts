import { GITHUB_CREDENTIAL_FIELDS, type GitHubCreds } from '../../shared/github-schema';
...
export function validateConfig(raw: unknown): ValidateResult<Config> {
  ...
  if (!isPlainObject(creds)) return { ok: false, reason: 'missing github credentials' };

  for (const field of GITHUB_CREDENTIAL_FIELDS) {
    if (field.required && typeof creds[field.key] !== 'string') {
      return { ok: false, reason: `missing required field: github.${field.key}` };
    }
  }
  const value: Config = {
    github: {
