...
export async function runClaudePass(opts: { prompt: string; model?: string; signal?: AbortSignal }): Promise<ClaudePassResult> {
  ...
  try {
    for await (const line of readLines(proc.stdout)) {
      ...
      if (frame.kind === 'result') {
        result = { ok: true, text: frame.text, tokens: frame.tokens, costUsd: frame.costUsd };
    ...
    const digest = stderrLines.slice(-5).join(' · ') || `claude exited ${exitCode} with no result frame`;
...
type Frame = { kind: 'result'; text: string; tokens: number; costUsd: number } | { kind: 'error'; message: string };
...
function parseFrame(line: string): Frame | null {
  ...
  if (typeof obj.subtype === 'string' && obj.subtype.startsWith('error_')) {
    return { kind: 'error', message: obj.subtype === 'error_max_turns' ? 'claude: max turns reached' : `claude: ${obj.subtype}` };
  }
...
export async function checkClaudeAvailable(): Promise<{ available: boolean; version?: string; error?: string }> {
  try {
    ...
    if (code !== 0) return { available: false, error: `claude --version exited ${code}` };
    return { available: true, version: out.trim() };
  } catch (e) {
    return { available: false, error: e instanceof Error ? e.message : 'claude CLI not found on PATH' };
  }
}
