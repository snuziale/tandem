...
import { Pass2OutputSchema, type FindingJson } from '../../../shared/finding-schema';
import { diffLineIndex } from '../../../shared/gh/patch';
import type { ReviewThread } from '../../../shared/review-types';
import { capFindings, extractJson, parseWithSchema, sanitizeFindings } from './parse';
...
describe('extractJson', () => {
  ...
  it('falls back to outermost braces', () => {
    expect(extractJson('Here you go: {"findings": []} hope that helps')).toBe('{"findings": []}');
  });
...
describe('parseWithSchema', () => {
  ...
  it('reports zod issues on shape mismatch', () => {
    const result = parseWithSchema('{"findings": [{"path": ""}]}', Pass2OutputSchema);
...
describe('sanitizeFindings', () => {
  const index = new Map([['src/a.ts', diffLineIndex('@@ -10,2 +10,3 @@\n ctx\n-old\n+new1\n+new2')]]);
  ...
  it('drops findings a human thread already covers', () => {
    ...
    const { kept, discarded } = sanitizeFindings([finding({ endLine: 11 }), finding({ endLine: 12 })], index, [thread]);
...
describe('capFindings', () => {
  ...
  it('enforces the total cap', () => {
    const input = Array.from({ length: 12 }, (_, i) => finding({ title: `f${i}`, severity: 'risk' }));
