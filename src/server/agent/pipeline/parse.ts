...
export function sanitizeFindings(
  ...
  const humanAnchors = new Set(
    threads
      .filter((t) => t.line !== null)
      .map((t) => `${t.path}:${t.side}:${t.line}`),
  );
  ...
  for (const finding of findings) {
    ...
    const anchored =
      !!side &&
      side.has(finding.endLine) &&
      (finding.startLine === undefined || finding.startLine <= finding.endLine);
    const humanCovered = humanAnchors.has(
      `${finding.path}:${finding.side}:${finding.endLine}`,
    );
