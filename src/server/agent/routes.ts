...
const FINDING_STATES: FindingState[] = ['proposed', 'staged', 'edited', 'dismissed', 'posted', 'stale'];

async function handleFindingState(req: Request, runId: string, findingId: string): Promise<Response> {
  const body = await parseJsonBody(req);
  const state = isPlainObject(body) ? body.state : undefined;
  if (typeof state !== 'string' || !FINDING_STATES.includes(state as FindingState)) {
    return Response.json({ error: 'expected { state: FindingState }' }, { status: 400 });
  }
  try {
    const run = await transitionFinding(runId, findingId, state as FindingState);
    return Response.json({ run });
  } catch (e) {
    return Response.json({ error: e instanceof Error ? e.message : 'transition failed' }, { status: 409 });
  }
}
