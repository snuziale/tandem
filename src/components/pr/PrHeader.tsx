import { cn, Tooltip, TooltipContent, TooltipPortal, TooltipTrigger } from '@uipath/apollo-wind';
  ...
  return (
    <div className="border-b border-border px-4 py-2.5 space-y-1.5">
      ...
      <div className="flex items-center gap-3 text-xs font-mono flex-wrap">
        ...
        <span>
          <span className="text-emerald-400">+{pr.additions}</span> <span className="text-red-400">−{pr.deletions}</span>
        </span>
        ...
        <span className="flex items-center gap-2">
            ...
            <Tooltip key={`${check.name}-${i}`}>
              <TooltipTrigger asChild>
                <span className="flex items-center gap-1 text-muted-foreground">
                  <span className={cn('inline-block w-1.5 h-1.5 rounded-full', CHECK_DOT[check.status])} />
          ...
          {pr.checkRuns.length > 8 ? <span className="text-muted-foreground">+{pr.checkRuns.length - 8}</span> : null}
        </span>
        <span className="border border-border rounded px-1.5 py-0.5 text-muted-foreground">{pr.headSha.slice(0, 7)}</span>
      </div>
    </div>
  );
}
