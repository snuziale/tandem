...
import type { PendingReview, PrId, ReviewVerdict } from '../../shared/review-types';
...
const VERDICTS: Array<{ value: ReviewVerdict; label: string; activeClass: string }> = [
  { value: 'APPROVE', label: 'Approve', activeClass: 'border-emerald-400/60 text-emerald-400' },
  { value: 'COMMENT', label: 'Comment', activeClass: 'border-border text-foreground' },
  { value: 'REQUEST_CHANGES', label: 'Request changes', activeClass: 'border-red-400/60 text-red-400' },
];

export function ReviewTray({ prId, review, onVerdict, onSummary, submitDisabledReason }: Props) {
  ...
  const submit = useMutation({
    mutationFn: () => submitPr(prId, { verdict: verdict ?? 'COMMENT', summaryBody: review?.summaryBody ?? '' }),
    onSuccess: ({ url }) => {
  ...
  const canSubmit = !!verdict && (comments.length > 0 || verdict !== 'COMMENT' || !!review?.summaryBody?.trim());
  ...
  return (
    <>
      <div className="border-t border-border px-4 py-2 flex items-center gap-3 shrink-0 bg-background">
        <span className="text-xs font-mono">
          ...
          <span className="text-muted-foreground">
            ...
            {comments.length > 0 ? (
              <>
                {' '}
                · {humanCount} yours{agentCount > 0 ? <span style={{ color: 'var(--tandem-agent)' }}> · {agentCount} agent</span> : null}
              </>
            ) : null}
          </span>
        </span>
        ...
        >
          Submit review <span className="ml-1.5 text-[10px] opacity-70">⌘↵</span>
        </Button>
      </div>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Posts ONE GitHub review as you — {comments.length} comment{comments.length === 1 ? '' : 's'} + verdict{' '}
              <span className="font-mono">{verdict}</span>. The author gets a single notification.
            </DialogDescription>
          </DialogHeader>
          <Textarea
          ...
          <DialogFooter>
            ...
            <Button onClick={() => submit.mutate()} disabled={submit.isPending}>
              {submit.isPending ? 'Submitting…' : `Submit ${verdict?.toLowerCase().replace('_', ' ')}`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
