...
describe("parseViewsJson", () => {
  it("accepts a valid array and fills id/position defaults", () => {
    const result = parseViewsJson(
      JSON.stringify([{ name: "Repo only", query: "is:pr repo:a/b" }]),
    );
  ...
  it("rejects malformed payloads with a pointed error", () => {
    ...
    expect(parseViewsJson("{}")).toEqual({
      error: "expected a top-level array of views",
    });
    expect(parseViewsJson("[]")).toEqual({
      error: "at least one view is required",
    });
    expect(parseViewsJson(JSON.stringify([{ name: "", query: "x" }]))).toEqual({
      error: "view 0: name and query are required strings",
    });
    expect(
      parseViewsJson(
        JSON.stringify([{ name: "a", query: "q" }, { name: "b" }]),
      ),
    ).toEqual({
