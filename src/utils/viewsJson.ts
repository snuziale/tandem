...
export function parseViewsJson(text: string): { views: SavedView[] } | { error: string } {
  let raw: unknown;
  try {
    raw = JSON.parse(text);
  } catch (e) {
  ...
  if (!Array.isArray(raw)) return { error: 'expected a top-level array of views' };
  const views: SavedView[] = [];
  for (let i = 0; i < raw.length; i++) {
    const entry = raw[i];
    if (!isPlainObject(entry) || typeof entry.name !== 'string' || !entry.name.trim() || typeof entry.query !== 'string' || !entry.query.trim()) {
