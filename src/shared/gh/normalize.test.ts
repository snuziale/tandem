...
import { checkRunOf, normalizeFile, normalizePr, normalizeThread, rollupOf } from './normalize';
            ...
            statusCheckRollup: {
              ...
              contexts: {
                nodes: [
                  ...
                  { __typename: 'StatusContext', context: 'visual-diff', state: 'PENDING' },
                ],
              },
            },
          },
...
describe('checkRunOf', () => {
  it('treats non-success completions as failure', () => {
    expect(checkRunOf({ __typename: 'CheckRun', name: 'x', status: 'COMPLETED', conclusion: 'TIMED_OUT' }).status).toBe('failure');
    expect(checkRunOf({ __typename: 'CheckRun', name: 'x', status: 'COMPLETED', conclusion: 'CANCELLED' }).status).toBe('failure');
    expect(checkRunOf({ __typename: 'CheckRun', name: 'x', status: 'COMPLETED', conclusion: 'NEUTRAL' }).status).toBe('neutral');
  });
  it('maps incomplete runs and pending status contexts to pending', () => {
    expect(checkRunOf({ __typename: 'CheckRun', name: 'x', status: 'QUEUED', conclusion: null }).status).toBe('pending');
    expect(checkRunOf({ __typename: 'StatusContext', context: 'ci', state: 'EXPECTED' }).status).toBe('pending');
  });
});
...
describe('normalizePr', () => {
  it('normalizes a search node', () => {
    ...
    expect(pr!.checkRuns.map((c) => c.status)).toEqual(['success', 'failure', 'pending', 'pending']);
...
function thread(overrides: Partial<GqlReviewThread> = {}): GqlReviewThread {
  return {
    ...
    isResolved: false,
    isOutdated: false,
    comments: { nodes: [{ id: 'C_1', author: { login: 'sflorentino' }, body: 'Nice.', createdAt: '2026-08-21T09:00:00Z' }] },
    ...overrides,
  };
}
...
describe('normalizeFile', () => {
  ...
  it('flags binaries (no patch, zero line changes)', () => {
    expect(normalizeFile({ ...base, filename: 'logo.png', patch: undefined, additions: 0, deletions: 0, changes: 0 }).isBinary).toBe(true);
    expect(normalizeFile(base).isBinary).toBe(false);
  });

  it('flags generated paths', () => {
    expect(normalizeFile({ ...base, filename: 'pnpm-lock.yaml' }).isGenerated).toBe(true);
