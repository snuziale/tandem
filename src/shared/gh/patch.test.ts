...
describe('buildFilePatch', () => {
  ...
  it('marks added and removed files with /dev/null sides', () => {
    expect(buildFilePatch({ ...base, status: 'added' })).toContain('--- /dev/null');
    expect(buildFilePatch({ ...base, status: 'added' })).toContain('new file mode');
    expect(buildFilePatch({ ...base, status: 'removed' })).toContain('+++ /dev/null');
  });

  it('records renames', () => {
    const patch = buildFilePatch({ ...base, status: 'renamed', previousPath: 'src/old.ts' })!;
